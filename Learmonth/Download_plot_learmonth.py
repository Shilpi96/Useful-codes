####### important to keep srs_data.py code to keep in the same directory
import execnet
import matplotlib
import pandas as pd
...
def fill_nan(arr):
    ...
    If the entire array is NaN or empty, returns the array as is.
    """
...
def srs_to_pd(srs_file, pd_file, bkg_sub=False, do_flag=True, flag_cal_time=True):
    ...
    # raw_data contains: [a_band_data, b_band_data, timestamps]
    ...
    # Build a DataFrame with rows = timestamps and columns = frequencies.
    ...
    # Transpose so that rows are frequencies and columns are timestamps.
    full_band_data = full_band_data.transpose()
    freq_index = full_band_data.index  # frequencies (e.g., 802 channels)
    ...
    # Rebuild the DataFrame: rows = frequencies, columns = timestamps.
...
def plot_learmonth_DS(pd_file, save_file="", start_time="", end_time=""):
    ...
    if len(sel_timestamps) == 0:
        print("No timestamps found in the specified range.")
        sel_timestamps = timestamps

    sel_data = pd_data[sel_timestamps]

    matplotlib.rcParams.update({"font.size": 15})
    # Generate tick indices (ensuring at least 10 ticks or so).
    ...
    # Use the middle timestamp to build a title.
...
if __name__ == "__main__":
    main() #### run for example python download_plot_learmonth_data.py --starttime "24-10-2024 00:00:00" --endtime "24-10-2024 23:59:59"
